...
export class FetchService {
    async execute(path: string, request: Request) {
        ...
        if (!path) {
            throw new HttpException('Specific path is required to execute a fetch call',
        ...
        const requestHeaders = optionToken? { ... headers, access_token: optionToken } : headers;
        ...
        if (!resp.ok) {
            throw new HttpException(
                `Failed making ${method} request to ${path}: ${await resp.text()}`,
                resp.status,
            );
        }
