import { Driver, ObjectLiteral, QueryRunner } from 'typeorm';
import { RelationIdLoadResult } from 'typeorm/query-builder/relation-id/RelationIdLoadResult';
import { ColumnMetadata } from 'typeorm/metadata/ColumnMetadata';
import { Alias } from 'typeorm/query-builder/Alias';
import { RelationCountLoadResult } from 'typeorm/query-builder/relation-count/RelationCountLoadResult';
...
export class RawSqlResultsToEntityTransformer {
  ...
  protected transformRawResultsGroup(
    ...
    const hasOnlyVirtualPrimaryColumns =
      metadata.primaryColumns.filter(column => column.isVirtual === false)
  ...
  protected transformRelationIds(
    ...
    this.rawRelationIdResults.forEach(rawRelationIdResult => {
      ...
      const idMaps = rawRelationIdResult.results
        .map(result => {
          const entityPrimaryIds = this.extractEntityPrimaryIds(
            relation,
            result,
          );
          ...
          if (
            columns.length === 1 &&
            rawRelationIdResult.relationIdAttribute.disableMixedMap === false
          ) {
            if (relation.isOneToMany || relation.isOneToOneNotOwner) {
    ...
    this.rawRelationCountResults
      ...
      .forEach(rawRelationCountResult => {
        ...
        if (relation.isOneToMany) {
          referenceColumnName = relation.inverseRelation!.joinColumns[0]
